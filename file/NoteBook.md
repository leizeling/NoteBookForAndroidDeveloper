...
标签类有许多的缺点，它们中充斥着样板代码，包括枚举声明、标签域以及条件语句。由于多个实现乱七八糟的记在单个类中，破坏了可读性。标签类过于冗长、容易出错、并且效率低下。

标签类正是对类层次的一种简单的效仿。使用类的层次即可解决。即定义公共的抽象父类，包含抽象的公共方法，各个具体的自类进行继承实现。

#### 第24条：静态成员类优于非静态成员类

嵌套类（nested class）是指定义在另一个类的内部的类。包括以下几种：

* 静态成员类（static member class），等价于一般外部类，实例不会持有外部类实例，所以当不需要引用外部成员时建议都使用静态成员类，这样可以防止在非静态内部类中容易出现的外部类内存泄漏。
* 非静态成员类（non static member class）
* 匿名类（anonymous class），最常见的就是调用一个方法，然后最后一个参数是回调接口，直接在调用时使用匿名类进行实现。
* 局部类（local class），定义在方法中的类。

总而言之，共有四种不同的嵌套类，每一种都有自己的用途。如果一个嵌套类需要在单个方法之外仍然可见，或者它太长了，不适合放在方法内部，就应该使用成员类。如果成员类的每个实例都需要一个指向其外围实例的引用，就要把成员类做成非静态的；否则，就做成静态的。假设这个嵌套类属于方法的内部，如果你只需要在一个地方创建实例，并且已经有一个预置的类型可以说明这个类的特征，就要把它做成匿名类，；否则，就做成局部类。
